...
# Вам необходимо написать программу, которая генерирует
# список из чисел от 0 до N (не включая N).
# Например, если N — это 5, то нужно работать со списком 0, 1, 2, 3, 4.
#
# Также есть дополнительное условие.
# При заполнении списка нужно выполнить одно из двух действий с каждым числом:
#
# если индекс числа чётный (или 0), то вместо числа необходимо взять 1;
# если индекс числа нечётный, то вместо числа необходимо
# взять остаток от деления этого числа на 5 (число % 5).
...
result_list = [1 if index % 2 == 0 else index % 5 for index in range(N)]
