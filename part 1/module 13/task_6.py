...
def compute_safe_depth(threshold):
    ...
    while abs(formula(mid_point)) >= threshold:
        if abs(formula(lower_limit)) < abs(formula(upper_limit)):
...
def main_function():
    ...
    print(
        f'Приблизительная глубина безопасной кладки: {compute_safe_depth(accept_danger_lvl)} м'
    )
